    ...
    public class Player : NetworkBehaviour
    {
        ...
        public void LoadPlayerStates(NetPlayerInfo playerState, NetPlayerInfo opponentState)
        {
            ...
            foreach (var player in players)
            {
                ...
                player.Value.currentCardInstanceId = player.Key.currentCardInstanceId;
        ...
        public virtual void OnEndTurn(EndTurnMessage msg)
        {
            ...
            {
                foreach (var card in zone.Value.cards.ToList())
                {
                    foreach (var stat in card.stats)
                    {
        ...
        public virtual void StopTurn()
        {
            ...
            NetworkClient.Send<StopTurnMessage>(msg);
        }

        public virtual void OnReceiveChatText(NetworkIdentity senderNetId, string text)
        {
        }
                ...
                if (activatedAbilities.Count > 0 && abilityIndex < activatedAbilities.Count)
                {
                    ...
                    if (cost != null)
                    {
                        ...
                        if (playerInfo.stats[payResourceCost.statId].effectiveValue >= statCost)
                        {
                            ...
                            NetworkClient.Send<ActivateAbilityMessage>(msg);
                        }
                    }
                }
            }
        ...
        [System.Obsolete("PlayCreatureCard is deprecated, please use PlayCard instead.")]
        public void PlayCreatureCard(RuntimeCard card, List<int> targetInfo = null)
        {
            PlayCard(card, targetInfo);
        }

        [System.Obsolete("PlaySpellCard is deprecated, please use PlayCard instead.")]
        ...
        {
            effectSolver.FightPlayer(netIdentity, cardInstanceId);

            var msg = new FightPlayerMessage();
            msg.attackingPlayerNetId = netIdentity;
            msg.cardInstanceId = cardInstanceId;
            NetworkClient.Send<FightPlayerMessage>(msg);
        }

        public void FightCreature(RuntimeCard attackingCard, RuntimeCard attackedCard)
        {
            effectSolver.FightCreature(netIdentity, attackingCard, attackedCard);
            ...
            NetworkClient.Send<FightCreatureMessage>(msg);
        }
