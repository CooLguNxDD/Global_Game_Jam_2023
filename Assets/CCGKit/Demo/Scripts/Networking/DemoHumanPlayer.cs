...
namespace CCGKit
{
    ...
    #pragma warning restore 649
        ...
        public override void OnStartLocalPlayer()
        {
            ...
            lifeStat.onValueChanged += (oldValue, newValue) => { gameUI.SetPlayerHealth(lifeStat.effectiveValue); };
        ...
        public override void OnStartGame(StartGameMessage msg)
        {
            ...
            var gameScreen = GameObject.FindObjectOfType<GameScreen>();
            if (gameScreen != null)
            {
                gameScreen.CloseWaitingWindow();
            }
        ...
        public override void OnStartTurn(StartTurnMessage msg)
        {
            ...
            if (msg.isRecipientTheActivePlayer)
            {
                ...
                var scene = GameObject.FindObjectOfType<GameScreen>();
        ...
        protected void UpdateHandCardsHighlight()
        {
            foreach (var card in playerHandCards)
            {
                if (card is CreatureCardView)
                {
                    if (boardZone.maxCards > 0 && boardZone.numCards == boardZone.maxCards)
                    {
                        card.SetHighlightingEnabled(false);
                        continue;
                    }
            ...
            if (cardType.name == "Creature")
            {
                ...
                RearrangeTopBoard(() =>
                {
                    ...
                    if (msg.targetInfo != null && msg.targetInfo.Length >= 2)
                    {
                        ...
                        effectSolver.MoveCard(opponentInfo.netId, opponentBoard.cards[opponentBoard.cards.Count - 1], "Hand", "Board", msg.targetInfo);
        ...
        public override void OnReceiveChatText(NetworkIdentity senderNetId, string text)
        {
            if (GameNetworkManager.Instance.IsSinglePlayer)
            {
                return;
        ...
        public override void OnPlayerDrewCards(PlayerDrewCardsMessage msg)
        {
            ...
            {
                var netCard = msg.cards[i];
                var runtimeCard = NetworkingUtils.GetRuntimeCard(netCard);
                handZone.AddCard(runtimeCard);
            }
            RearrangeHand();
            UpdateHandCardsHighlight();
